//num of lines: 65
export const array = [
"Always bring cinnamon buns on a deep-sea diving expedition.",
"She opened up her third bottle of wine of the night.",
"Being unacquainted with the chief raccoon was harming his prospects for promotion.",
"If you really strain your ears, you can just about hear the sound of no one giving a damn.",
"There aren't enough towels in the world to stop the sewage flowing from his mouth.",
"The tree fell unexpectedly short.",
"It was a slippery slope and he was willing to slide all the way to the deepest depths.",
"People generally approve of dogs eating cat food but not cats eating dog food.",
"I like to do my happy dance when all of the socks from the dryer matched.",
"Hang on, my kittens are scratching at the bathtub and they'll upset by the lack of biscuits.",
"He played the game as if his life depended on it and the truth was that it did.",
"Chocolate covered crickets were his favorite snack.",
"You should never take advice from someone who thinks red paint dries quicker than blue paint.",
"His get rich quick scheme was to grow a cactus farm.",
"I'm worried by the fact that my daughter looks to the local carpet seller as a role model.",
"The skeleton had skeletons of his own in the closet.",
"The grey of a knowledge becomes a cozy shelf.",
"A parrot can hardly be considered an ansate hip without also being a snow. Spandexes are printed weapons.",
"Recent controversy aside, an aground tax is an inch of the mind.",
"A science sees a pond as a bardy belief.",
"However, the first edgy ketchup is, in its own way, an epoxy.",
"The modest era comes from a massive satin.",
"A knife is a phone's dragonfly.",
"A headline of the mandolin is assumed to be a nephric office.",
"The great-grandfather of a particle becomes a motey advantage.",
"Framed in a different way, some tarry wrenches are thought of simply as cities.",
"The literature would have us believe that a jesting son is not but a plain.",
"The cecal hourglass comes from a shortcut error.",
"A methane of the whiskey is assumed to be a defined territory.",
"They were lost without the honied latex that composed their dime.",
"Extending this logic, before zippers, blizzards were only chicories.",
"Authors often misinterpret a metaphor as a sign of trouble, when in actuality it feels more like a yester flavor.",
"Just wakin' up in the mornin', gotta thank God, I don't know but today seems kinda odd.",
"No barkin' from the dog, no smog, and momma cooked a breakfast with no hog.",
"I got my grub on, but didn't pig out, finally, got a call from a girl I wanna dig out.",
"I gotta go 'cause I got me a drop top and if I hit the switch, I can make the ass drop.",
"Had to stop, at a red light, looking in my mirror, not a jacker in sight.",
"You 'bout to feel the chronicles of a bionical lyric lyrically splitting dismissing.",
"I'm on a mission of just hitting now it's written and kitten hitting wit mittens.",
"Bust the move and then swerve.",
"Serve words with nerve embedded I said it word.",
"The flow pro poetical with skills only a vet'll know.",
"Better know where's the wetter flow that's on point like decimals.",
"Looking for the pink in poo poo. I thought you knew too.",
"Stone style is of stamina jamming ta while we planning ta jam.",
"Amateur damager managing damaging mics, men and even mannequins.",
"Today is the day I'll finally know what brick tastes like.",
"He took one look at what was under the table and noped the hell out of there.",
"She had a habit of taking showers in lemonade.",
"The teenage boy was accused of breaking his arm simply to get out of the test.",
"Had he known what was going to happen, he would have never stepped into the shower.",
"Traveling became almost extinct during the pandemic.",
"The llama couldn't resist trying the lemonade.",
"The glacier came alive as the climbers hiked closer.",
"When transplanting seedlings, candied teapots will make the task easier.",
"He had concluded that pigs must be able to fly in Hog Heaven.",
"Nancy decided to make the porta-potty her home.",
"He fumbled in the darkness looking for the light switch, but when he finally found it there was someone already there.",
"Behind the window was a reflection that only instilled fear.",
"However, unknowingly taken up sleepwalking as a nighttime hobby.",
"Shit, my fingers are hurting again.",
"Every day, I like to say good morning to the sexy mf in the mirror.",
"Typing vigorious is one way to get arthritis.",
"Whoaaaaa, slow down there cowboy.",
"In Pixar's Up, Carl and Ellie break into their Paradise Falls savings jar in order to pay for Carl's hospital bill, this is actually a clever reference to the fact that the American healthcare system is literal garbage.",
];